...
const App = () => {
  ...
  const closePopups = () => {setIsOpenTips(false)};
  ...
  const deleteSavedFilms = (movie) => {
    const movieId =
      movie._id || savedMovies.find((item) => item.movieId === movie.movieId)._id;
    api.deleteMovie(movieId)
      .then(() => {
        setSavedFilms((savedingFilms) => savedingFilms.filter((savedMovie) => savedMovie._id !== movieId)
        );
      })
      .catch(console.error);
  };

  const getAllMovies = () => {getMovies()
      .then((nextFilms) => {
  ...
  function arrMoviesFromStorage() {return JSON.parse(localStorage.getItem('movies') ?? '[]')}

  function checkSavedMovies(movie) {
    return savedMovies.some((item) => item._id === movie._id || item.movieId === movie.movieId);
  }
  ...
  const updating = (name, email) => {return api
      ...
      <MoviesProvider>
        <div className="app">
          {headerPaths.includes(path) && (
            <Header isLoggedInto={isLoggedInto} isMobileOpened={isMobileOpened}/>)}
          <Routes>
            <Route element={
            <ProtectedRoutes isLoggedInto={!isLoggedInto} />}>
              <Route path="/signin" element={
              <Login isLoggedInto={isLoggedInto} logining={logining} errorMessage={loginError} title="Вход" buttonText="Войти"/>}
              ></Route>
              <Route path="/signup" element={
              <Register isLoggedInto={isLoggedInto} registering={registering} title={'Регистрация'} buttonText={'Зарегистрироваться'}/>}
              ></Route>
            </Route>
            ...
            <Route element={<ProtectedRoutes isLoggedInto={isLoggedInto} />}>
              ...
              <Movies getMovies={getAllMovies} movies={movies} addMovie={plusSavedFilms} onDeleteFilm={deleteSavedFilms} checkSavedMovies={checkSavedMovies}/>}
              ></Route>
              <Route path="/saved-movies" element={<SavedMovies movies={savedMovies} onDeleteFilm={deleteSavedFilms} checkSavedMovies={checkSavedMovies}/>}
              ></Route>

              <Route path="/profile" element={<Profile loginingOut={logOut} updating={updating}/>}
              ></Route>
          ...
          <InfoTooltip isRegister={isStatus} isOpen={isOpenInfoTooltip} onClose={closePopups} alt={'Статус'}/>
        </div>
      </MoviesProvider>
